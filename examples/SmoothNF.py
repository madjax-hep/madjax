...
def tanh_bijector(x, cond, eps=1e-2):
    ...
    def _call(x):
        """Evaluate forward."""
        return 0.5 * (x[..., None] + (1. - eps) * jnp.tanh((x[..., None] - b) * a)).sum(axis=-1)
...
def tanh_bijector_inverse(x, cond):
  _inverse = wrap_with_inverse(
      tanh_bijector,
      partial(bisect_inverse, left_bound=0., right_bound=1.)
  )
...
def smoothfun_bijector(x, cond, beta=2.0, eps=1e-6):
    ...
    def _call(x):
        def rho(z):
            ...
            _rho_val = jnp.where( z>0., jnp.exp(-1.0 / ((alpha+1.0)*jnp.power(z, beta))), 0.)
            return _rho_val
            #_rho_val = jnp.exp(-1.0 / (alpha*jnp.power(z, beta)))
            #return jnp.where(z>0., _rho_val, 0.)        
        
        def sigma(z):
            return rho(z) / (rho(z) + rho(1.-z))
        
        def g(z):
            return (1. - c)*sigma(a * (z - b) + 0.5) + (c + eps)*z
...
def smoothfun_bijector_inverse(x, cond):
    _inverse = wrap_with_inverse( smoothfun_bijector,
                                      partial(bisect_inverse,
                                                  left_bound=0.,
                                                  right_bound=1.)
                                      )
    return _inverse(x, cond)
...
class SmoothFlowLayer(nn.Module):
    ...
    def setup(self):
        ...
        self.conditioner = MLPconditioner(num_bij = self.num_biject, layer_widths = mlp_widths) 
...
class SmoothNormalizingFlow(nn.Module):
    ...
    def setup(self):
        self.flows = list(SmoothFlowLayer(num_biject = self.num_biject,
    ...
    def logprob(self, x):
        ...
        base_logprob = jax.scipy.stats.uniform.logpdf(z, loc=0, scale=1).sum(axis=tuple(range(1,z.ndim))).reshape(-1,1)
