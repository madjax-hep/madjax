...
# ================================================
def generate_phase_space_inputs(
    initial_masses,
    final_masses,
    beam_Es,
    beam_types=(1, 1),
    ...
    # Sanity check
    if _inputs["correlated_beam_convolution"] and _inputs[
    ...
    _inputs["dim_name_to_position"] = {d.name: i for i, d in enumerate(dimensions)}
    _inputs["position_to_dim_name"] = {
        v: k for (k, v) in _inputs["dim_name_to_position"].items()
    }
...
def boost_to_lab_frame(PS_point, n_initial, xb_1, xb_2):
    ...
    if n_initial == 2 and (xb_1 != 1.0 or xb_2 != 1.0):
...
def rho(M, N, m):
    ...
    Msqr = jnp.power(M, 2)
    return jnp.power(
        (Msqr - jnp.power((N + m), 2)) * (Msqr - jnp.power((N - m), 2)), 0.5
    ) / (8.0 * Msqr)
...
def setInitialStateMomenta(PS_inputs, output_momenta, E_cm):
    ...
    if n_initial == 1:
        output_momenta[0] = LorentzVector([initial_masses[0], 0.0, 0.0, 0.0])
        return

    elif n_initial == 2:
        if initial_masses[0] == 0.0 or initial_masses[1] == 0.0:
            output_momenta[0] = LorentzVector([E_cm / 2.0, 0.0, 0.0, 1.0 * E_cm / 2.0])
            ...
            output_momenta[1] = LorentzVector([E2 / 2.0, 0.0, 0.0, (-1.0) * Z / 2.0])
    return
...
def get_PS_point(PS_inputs, random_variables):
    ...
    random_variables = [
        min(max(rv, epsilon_border()), 1.0 - epsilon_border())
    ...
    PS_random_variables = [
        ...
        if position_to_dim_name[i].startswith('x_')
    ]
    ...
    if abs(beam_types[0]) == abs(beam_types[1]) == 1:
        ...
        tau_min = (
            max(tot_final_state_masses, absolute_Ecm_min()) / collider_energy
        ) ** 2
        tau_max = 1.0
...
def generateKinematics(PS_inputs, E_cm, random_variables):
    ...
    if n_final == 1:
        ...
        output_momenta.append(LorentzVector([mass / 2.0, 0.0, 0.0, 1.0 * mass / 2.0]))
...
def generateIntermediatesMassive(n_final, masses, M, E_cm, random_variables):
    ...
    weight *= 8.0 * rho(M[n_final - 2], masses[n_final - 1], masses[n_final - 2])

    for i in range(2, n_final):
        weight *= (
            rho(M[i - 2], M[i - 1], masses[i - 2]) / rho(K[i - 2], K[i - 1], 0.0)
        ) * (M[i - 1] / K[i - 1])
...
def invertKinematics(PS_inputs, E_cm, momenta):
    ...
    for i in range(n_initial, n_final + 1):
        ...
        p.boost(boost_vec)
        random_variables[n_final - 2 + 2 * (i - n_initial)] = (p.cosTheta() + 1.0) / 2.0
        phi = p.phi()
        if phi < 0.0:
            phi += 2.0 * math.pi
        random_variables[n_final - 1 + 2 * (i - n_initial)] = phi / (2.0 * math.pi)
...
def invertIntermediatesMassless(n_final, K, E_cm, random_variables):
    ...
    for i in range(2, n_final):
        u = (K[i - 1] / K[i - 2]) ** 2
        random_variables[i - 2] = (n_final + 1 - i) * jnp.power(u, n_final - i) - (
            n_final - i
        ) * jnp.power(u, n_final + 1 - i)
